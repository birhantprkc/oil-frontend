...
> 任何前端代码实现或重构都应读取；本文件负责模块、组件、Hook、函数、类型、数据来源、样式所有权和旧实现清理。
...
- 既有实现只有在职责、行为、数据和样式所有权正确时才值得复用；存在时间长、使用页面多或位于共享目录，都不能证明它是正确模式。
...
- 每段样式必须归属于基础规则、共享组件或明确的业务模块；在样式所有权上，页面只负责区域布局，不接管组件内部视觉。
- 项目已有的样式组织方式符合所有权边界时才沿用；如果业务样式无边界集中、互相覆盖或无法随组件删除，就停止扩大全局文件，重新建立组件或业务模块的样式所有权。
...
修改共享组件、函数或样式后，按 [前端验证规则](verification-contract.md#使用位置检查) 检查全部使用位置、默认模式、各用途模式和旧实现，并报告未验证项。
