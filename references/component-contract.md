...
- 在共享组件内部维护默认视觉；调用方可使用组件明确公开的 `className`、slot、尺寸属性或 CSS 变量扩展点，遵守其用途与项目 Token。仅有参数可传入，不代表任意覆盖都符合契约。
...
- 用实例样式掩盖共享默认行为或父布局的错误，或超出组件公开扩展范围覆盖内部视觉和结构。
- `!important` 修补组件实例。
- 页面专属选择器侵入共享组件内部。
- 使用任意数值绕过统一设计变量。
...
- 绕过弹窗公开接口重排内部区域，破坏标题、内容滚动区和操作区的约定。
...
- 每段样式归属于基础规则、共享组件或明确业务模块；页面负责区域布局与公开扩展点的组合，不侵入组件内部。
...
- 实例尺寸通过父布局或组件公开尺寸接口表达，不用“刚好适配截图”的宽高掩盖布局缺陷；固定尺寸的适用条件见 [视口与弹窗规则](viewport-and-dialog-contract.md#内容驱动尺寸)，只在处理尺寸问题时读取。
