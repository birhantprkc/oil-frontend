...
> 当任务涉及展示与编辑切换、表单、行内编辑、批量工作表或选择流程时读取；纯浏览结构由集合与详情契约负责。

## 展示态与编辑态

资源目录和只读详情使用文字、资源身份、图片和状态组件。表单控件、上传区及保存操作只在用户明确开始修改、选择，或进入以批量配置为核心的工作表后出现。

禁止：

- 用只读或禁用的表单控件展示普通信息；
- 让资源目录、卡片和只读详情默认处于编辑态；
...
- 系统已经知道、上一步已经提供、能够继承或可靠推导的信息直接带入；只有字段属于当前任务对象且当前表单拥有编辑职责时才允许修改，否则展示来源并使用所有者页面或共享编辑器修改。
...
当前任务本身是连续配置多个同构任务时，可以让多行持续处于编辑态，但必须同时满足：
...
不要因为页面里出现多个输入框就判定交互错误。先判断它是资源目录，还是用户明确进入的批量工作台。
...
资源目录中的选择器关闭后恢复为资源身份展示；可编辑工作表的选择触发器可以持续可见，但展开选项仍按需出现。

## 数据所有权

先确定字段属于哪个业务对象，再决定编辑入口：

- 所有者页面或共享对象编辑器负责修改完整数据；
- 消费页面默认展示来源对象和当前引用值；
- 消费页面可以选择本次任务使用的子集，但不得复制来源对象的完整编辑表单；
- 需要修改来源数据时，打开同一个共享编辑器或前往所有者页面；
- 返回消费页面后保留当前位置、任务数据和最新引用值。

禁止在多个页面分别维护同一字段的编辑逻辑、校验和保存入口。

## 选择与候选

- 存在明确默认选择时，先按 [资源识别契约](resource-recognition-contract.md#已选结果) 展示当前选择，通过“更改选择”按需打开选择器；展开候选后隐藏只读摘要，禁止同时重复显示同一批对象。
...
跨步骤数据带入、返回位置和原对象更新统一遵循 [作用域与状态真实性契约](scope-and-state-integrity-contract.md#流程连续性)。
