...
> 当任务涉及列表、卡片、详情、表格或批量操作时读取；字段编辑和数据请求由对应契约负责。
...
- 可编辑工作表按任务阶段组织相邻列，并固定身份列与操作列；横向滚动遵循 [视口与弹窗契约](viewport-and-dialog-contract.md#滚动所有权)。
- 集合内媒体只提供轻量预览；播放对象离开当前筛选或分页时停止播放，除非页面提供始终可见的播放控制。

对象跨阶段的更新与派生规则遵循 [作用域与状态真实性契约](scope-and-state-integrity-contract.md#流程连续性)。
