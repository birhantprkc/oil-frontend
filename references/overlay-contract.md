...
> 当任务涉及下拉选择、菜单、日期面板、提示、上下文菜单或其他依附触发器的浮层时读取；完整弹窗、抽屉和页面布局由视口与弹窗契约负责。

## 所有权

- 定位、碰撞检测、层级、尺寸边界和关闭行为由共享弹层组件负责。
- 使用弹层的页面只提供触发器、内容和用途模式，不传当前位置专用的偏移或宽高。
- 表格、卡片和滚动容器不负责约束浮层边界。
...
- 紧凑菜单按内容宽度，不吸收页面剩余空间。
- 内容超出普通下拉能够清楚承载的范围时，按 [视口与弹窗契约](viewport-and-dialog-contract.md#承载方式弹窗抽屉与完整页面) 选择弹窗或完整工作区，不继续扩大普通下拉。
